...
/* global chrome, rise, getText, longToNormalAmount, sourceUrl, sourcePriceUrl, riseRegex */
    ...
    if (scriptName === 'rise') {
      ...
      chrome.storage.local.get(
        ...
        item => {
          ...
          if (item.useSource) {
            source =
              item.useSource.toString() === '3'
                ? item.source3
                : item.useSource.toString() === '2'
                ? item.source2
                : sourceUrl;
          } else {
            source = sourceUrl;
          ...
          if (item.useSourcePrice) {
            sourcePrice =
              item.useSourcePrice.toString() === '2'
                ? item.sourcePrice2
                : sourcePriceUrl;
          } else {
          ...
          if (a === 'y' || w.toString() !== '1') {
            if (t && Array.isArray(t) && t.reverse().length > 0) {
              lastMatchIds = (t[1] ? [...t[0], ...t[1]] : t[0]).map(
          ...
          setTimeout(() => {
            checkPrice(
              item.alertPriceChangeOnStartup &&
                item.alertPriceChangeOnStartup.toString() === '1'
            );
            checkAccounts(true, true);
            if (
              item.checkOfflineMessages &&
              item.checkOfflineMessages.toString() === '1' &&
  ...
  chrome.storage.local.get(
    ...
    item => {
      ...
      if (!item.alertPriceChangeOnStartup)
        initObject.alertPriceChangeOnStartup = '2';
          ...
          response => {
            if (Array.isArray(response) && response.length > 0) {
              ...
              for (let i = 0; i < response.length; i++) {
                ...
                let posResults = response[i].filter(
                  c =>
                    addresses.indexOf(c.recipientId) !== -1 &&
                    addresses.indexOf(c.senderId) === -1 &&
                    lastMatchIds.indexOf(c.id) === -1
                );
                ...
                let negResults = response[i].filter(
                  c =>
                    addresses.indexOf(c.senderId) !== -1 &&
                    lastMatchIds.indexOf(c.id) === -1
                );
...
function checkAccounts(
  ...
  chrome.storage.local.get(
    ...
    item => {
      ...
      if (addresses.length > 0) {
        fetchAccountInfo(addresses, includeDelegateInfo, response => {
          if (Array.isArray(response) && response.length > 0) {
            // if all addresses were invalid, response array is [0]
              ...
              if (response[i] !== null && Object.keys(response[i]).length > 0) {
                try {
                  if (allowUnconfirmedBalance) {
                    amountObj[`address${i + 1}amount`] =
                      response[i].account.unconfirmedBalance;
                  } else {
                    amountObj[`address${i + 1}amount`] =
                      response[i].account.balance;
                  }
                  twosigObj[`address${i + 1}twosig`] =
                    response[i].account.secondSignature.toString() === '1'
                      ? '1'
                      : '';
                } catch (e) {
                  amountObj[`address${i + 1}amount`] = -1;
                }
                if (includeDelegateInfo) {
                  try {
                    delegatesObj[`address${i + 1}delegate`] =
                      response[i].delegates[0].username;
                  } catch (e) {}
                  try {
                    delegatesProdObj[`address${i + 1}delegateProd`] =
                      response[i].delegates[0].productivity;
                  } catch (e) {}
                  try {
                    nameObj[`address${i + 1}name`] =
                      response[i].delegate.username;
                  } catch (e) {}
                }
              }
            }
...
function checkPrice(alertOnStartup = false, callbackOnComplete = () => {}) {
  ...
  ajax(
    ...
    () => {
      ...
      notifyConnectionProblems(
        `${getText('source')} (${getText('prices')}): ${sourcePrice}`
      );
    },
    response => {
      if (Array.isArray(response)) {
        ...
        if (
          typeof resp === 'object' &&
          resp.id.toString().toUpperCase() === 'RISE'
        ) {
          chrome.storage.local.set(
            { riseusd: resp.price_usd, risebtc: resp.price_btc },
            () => {
              callbackOnComplete(resp);
              if (startup && alertOnStartup) {
                let message;
                if (
                  resp.percent_change_1h !== 'undefined' &&
                  resp.percent_change_24h !== 'undefined'
                ) {
                  const change1H =
                    parseFloat(resp.percent_change_1h, 10) || 0.0;
                  const change24H =
                    parseFloat(resp.percent_change_24h, 10) || 0.0;
                  message = `${
                    change1H >= 0 ? `1h: +${change1H}%` : `1h: ${change1H}%`
                  }\n${
                    change24H >= 0
                      ? `24h: +${change24H}%`
                      : `24h: ${change24H}%`
      ...
      if (response.transactions && response.transactions.length > 0) {
        chrome.storage.local.get(
          ...
          item => {
            ...
            if (watchmessages === '1') {
              // sending to oneself, voting, registering a delegate or second signature should only count as 1 transaction (instead of 2)
              posResults = resp.filter(
                c =>
                  addresses.indexOf(c.recipientId) !== -1 &&
                  addresses.indexOf(c.senderId) === -1 &&
                  lastMatchIds.indexOf(c.id) === -1
              );
              negResults = resp.filter(
                c =>
                  addresses.indexOf(c.senderId) !== -1 &&
                  lastMatchIds.indexOf(c.id) === -1
              );
              ...
              posResults = resp.filter(
                c =>
                  addresses.indexOf(c.receiverId) !== -1 &&
                  addresses.indexOf(c.senderId) === -1 &&
                  lastMatchIds.indexOf(c.id) === -1
              );
              results = posResults;
...
function createNotification(
  ...
  const title = isPos
    ? `${getText('received')}: ${a} RISE`
    : `${getText('sent')}: ${a} RISE`;
  const message =
    length > 1
      ? `${getText('n_therewere')} ${length.toString()} ${getText(
          'n_transactions'
        )}.`
      : `${getText('n_therewas')} 1 ${getText('n_transaction')}.`;
  // store to latest results object; if transactions object has 10 entries, then also discard the oldest entry
  const logmessage = `${title} (${length.toString()} ${
    length === 1 ? getText('n_transaction') : getText('n_transactions')
  })`;
  const allmessages =
    item.messages.length + 1 < 11
      ? item.messages.concat([logmessage])
      : item.messages.concat([logmessage]).slice(1);
  item.messages = allmessages;
  const transfers =
    item.transactions.length + 1 < 11
      ? item.transactions.concat([r])
      : item.transactions.concat([r]).slice(1);
  item.transactions = transfers;
  isPos
    ? chrome.browserAction.setBadgeBackgroundColor({ color: '#22AB23' })
    : chrome.browserAction.setBadgeBackgroundColor({ color: '#D94523' });
  chrome.storage.local.set(
    { transactions: transfers, messages: allmessages },
    () => {
      // notify the user
      chrome.browserAction.setBadgeText({ text: length.toString() });
      const iconUrl = isPos
        ? 'images/rise_notification_posAmount.png'
        : 'images/rise_notification_negAmount.png';
      chrome.notifications.create({
        type: 'basic',
        iconUrl,
        title,
        ...
        if (validAddress) {
          ...
          if (includeDelegateInfo) {
            ...
            if (
              ...
              resultRegistration = await fetchRegistrationResult(
 ...
 * @param {function} callback Function to be called after the response has been processed
 */
async function getOfflineMessagesList(
  type = 1,
  blockheight = 1000000,
  addresses = [],
  callback = () => {}
