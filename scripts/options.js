...
function validateAddress(address, addressnr, allAddresses) {
  if (address === '') {
    ...
    document
      .getElementById(`address${addressnr + 1}note`)
    ...
    if (!isDuplicate(address, addressnr, allAddresses)) {
      ...
      document.getElementById(
        `address${addressnr + 1}note`
      ).textContent = getText('same_address');
      document
        .getElementById(`address${addressnr + 1}note`)
    ...
    document.getElementById(
      `address${addressnr + 1}note`
    ).textContent = getText('validation_error');
    document
      .getElementById(`address${addressnr + 1}note`)
      .removeAttribute('hidden');
    return false;
...
function restoreAllOptions() {
  ...
  document.getElementById('i_title').textContent =
    getText('extName') + ' ' + version;
  document.getElementById('i_addresses').textContent =
    'RISE ' + capitalize(getText('addresses'));
  const ids = [
    'i_address1',
    'i_address2',
    'i_address3',
  ...
  for (let i = 0; i < ids.length; i++) {
    document.getElementById(ids[i]).textContent = `${capitalize(
      getText('address')
    )} ${i + 1} :`;
  }
  ...
  document.getElementById('labelcheckstartup').textContent = getText(
    'checkstartup'
  );
  document.getElementById('labelcheckpricediff').textContent = getText(
    'show_pricechange'
  );
  document.getElementById('labelseparatemessage').textContent = getText(
    'separate_notifications'
  );
  document.getElementById('labelshowerrormessages').textContent = getText(
    'error_notifications'
  );
  document.getElementById('datasourcelabel1').textContent = `${getText(
    'source'
  )} 1 (RISE node):`;
  document.getElementById('datasourcelabel2').textContent = `${getText(
    'source'
  )} 2 (RISE node):`;
  document.getElementById('datasourcelabel3').textContent = `${getText(
    'source'
  )} 3 (RISE node):`;
  document.getElementById('pricesourcelabel1').textContent = `${getText(
    'source'
  )} 1 (${getText('prices')}):`;
  document.getElementById('pricesourcelabel2').textContent = `${getText(
    'source'
  )} 2 (${getText('prices')}):`;
  document.querySelectorAll('.canceloptions').forEach(elem => {
    elem.textContent = getText('button_cancel');
  });
  ...
  document.querySelectorAll('.urlinput').forEach(elem => {
    elem.setAttribute('placeholder', getText('opt_url_placeholder'));
  });
  document.getElementById('transactionsquestion').textContent = getText(
    'watch_question'
  );
      ...
      currentaddresses.map((val, index) => {
        ...
        if (val) {
          if (item[`address${index + 1}amount`].toString() !== '-1') {
            ...
            document.getElementById(`address${index + 1}amount`).textContent =
              (longToNormalAmount(
                parseInt(item[`address${index + 1}amount`], 10)
              ) || 0) + ' RISE';
            document
              .getElementById(`address${index + 1}amount`)
              .setAttribute('class', `ui horizontal label`);
            document
              .getElementById(`address${index + 1}url`)
              .setAttribute('href', `${explorerUrl}${val}`);
            document
              .getElementById(`address${index + 1}url`)
              .removeAttribute('hidden');
            if (item[`address${index + 1}twosig`]) {
              const icon2 = getLabeledIconElement('icon yellow key');
              const txtnode2 = getTextNode(
                '2 ' + getText('m_signatures').toLowerCase()
              );
              const target2 = document.getElementById(
                `address${index + 1}twosig`
              );
            ...
            if (item[`address${index + 1}delegate`]) {
              const prod = item[`address${index + 1}delegateProd`]
                ? ` (${item[`address${index + 1}delegateProd`]} %)`
                : '';
              const icon3 = getLabeledIconElement('icon blue pencil alternate');
              const txtnode3 = getTextNode(
                item[`address${index + 1}delegate`] + prod
              );
              const target3 = document.getElementById(
                `address${index + 1}delegate`
              );
            ...
            const icon4 = getLabeledIconElement(
              'icon red exclamation triangle'
            );
            const txtnode4 = getTextNode(getText('address_not_found'));
            const target4 = document.getElementById(
              `address${index + 1}amount`
            );
...
function saveAll() {
  const givenSources = [
    String(document.getElementById('source2').value).trim(),
    String(document.getElementById('source3').value).trim()
  ];
  const selectedSource = parseInt(
  ...
  const allAddressesValid =
    addresses
      .map((c, index) => validateAddress(c, index, addresses))
      .filter(c => !c).length === 0;
  ...
  if (allAddressesValid) {
    ...
    changeObj.watchmessages = document.querySelector(
      'input[name="watch"]:checked'
    ).value;
    changeObj.checkOfflineMessages = document.querySelector(
      'input[name="checkstartup"]'
    ).checked
      ? '1'
      : '2';
    changeObj.alertPriceChangeOnStartup = document.querySelector(
      'input[name="checkpricediff"]'
    ).checked
      ? '1'
      : '2';
    changeObj.allowmixedmessage = document.querySelector(
      'input[name="separatemessage"]'
    ).checked
      ? 'n'
      : 'y';
    changeObj.showerrormessages = document.querySelector(
      'input[name="showerrormessages"]'
    ).checked
      ? 'y'
      : 'n';
    changeObj.useSource = document.querySelector(
    ...
    changeObj.useSourcePrice = document.querySelector(
      'input[name="pricesource"]:checked'
    ).value;
    changeObj.sourcePrice2 = String(
      document.getElementById('sourceprice2').value
    ).trim();
...
function changeEraseButtonStatus(e) {
  if (e.target.checked) {
    document
      ...
      .setAttribute('class', 'ui black icon labeled small button');
  } else {
    document
      .getElementById('eraser')
      .setAttribute('class', 'ui black icon labeled small disabled button');
  }
}
...
document
  ...
  .addEventListener('change', changeEraseButtonStatus);
document
  .querySelectorAll('.canceloptions')
  .forEach(elem => elem.addEventListener('click', closeWindow));
